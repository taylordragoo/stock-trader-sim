...
function NewProduct() {
    ...
    const onImageMouseOver = (ref: any, fileName: string) => {
        if (ref === fileName) ref.current.style.display = 'flex';
    };

    const onImageMouseLeave = (ref: any, fileName: string) => {
        if (ref.id === fileName) {
    ...
    const onColorSelect = (colorName: string, i: number) => {
        if ((product.colors as any[])?.indexOf(colorName) !== -1) {
            (product.colors as any[]).splice((product.colors as any[]).indexOf(colorName), 1);
            setProduct((prevState) => ({ ...prevState, colors: (prevState.colors as any[]).filter((color) => color.name !== colorName) }));
        } else {
            setProduct((prevState) => ({ ...prevState, colors: [...(prevState.colors as any[]), colorName] }));
        }
    };

    const onUpload = (event: FileUploadUploadEvent | FileUploadSelectEvent) => {
        for (let file of event.files) {
            setProduct((prevState) => ({ ...prevState, images: (prevState.images as File[]).push(file) }));
        }
    };
            ...
            <div className="grid grid-nogutter flex-wrap gap-3 p-fluid">
                <div className="col-12 lg:col-8">
                    <div className="grid formgrid">
                        <div className="col-12 field">
                            <InputText type="text" value={product.name} onChange={(e) => setProduct((prevState) => ({ ...prevState, name: e.target.value }))} placeholder="Product Name" />
                        </div>
                        <div className="col-12 lg:col-4 field">
                            <InputText type="text" placeholder="Price" value={product.price?.toString()} onChange={(e) => setProduct((prevState) => ({ ...prevState, price: parseFloat(e.target.value) }))} />
                        </div>
                        <div className="col-12 lg:col-4 field">
                            <InputText type="text" placeholder="Product Code" value={product.code} onChange={(e) => setProduct((prevState) => ({ ...prevState, code: e.target.value }))} />
                        </div>
                        <div className="col-12 lg:col-4 field">
                            <InputText type="text" placeholder="Product SKU" value={product.sku as string} onChange={(e) => setProduct((prevState) => ({ ...prevState, sku: e.target.value }))} />
                        </div>
                        <div className="col-12 field">
                ...
                <div className="flex-1 w-full lg:w-3 xl:w-4 flex flex-column row-gap-3">
                    ...
                    <div className="border-1 surface-border flex justify-content-between align-items-center py-2 px-3 border-round">
                        ...
                        <InputSwitch checked={product.inStock as boolean} onChange={(e) => setProduct((prevState) => ({ ...prevState, inStock: e.value as boolean }))}></InputSwitch>
                    </div>
