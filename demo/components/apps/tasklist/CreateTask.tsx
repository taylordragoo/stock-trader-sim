...
function CreateTask() {
    ...
    const resetTask = () => {
        ...
        setTask({ id: parseFloat(taskId), name: '', description: '', status: 'Waiting' });
    };
                ...
                <div className="p-4">
                    <div className="grid p-fluid formgrid">
                        ...
                        <div className="col-12 field">
                            ...
                            <Editor value={task?.description} onTextChange={(e) => setTask((prevState) => ({ ...prevState, description: e.htmlValue ?? '' }))} style={{ height: '150px' }}></Editor>
                        </div>
                        <div className="col-6 field mt-0">
                            <label htmlFor="start" className="text-900 font-semibold">
                                Start Date
                            </label>
                            <Calendar dateFormat="yy-mm-dd" showTime={false} inputId="start" placeholder="Start Date" value={task?.startDate} onChange={(e) => setTask((prevState) => ({ ...prevState, startDate: e.value as string }))}></Calendar>
                        </div>
                        <div className="col-6 field mt-0">
                            <label htmlFor="end" className="text-900 font-semibold">
                                Due Date
                            </label>
                            <Calendar dateFormat="yy-mm-dd" showTime={false} inputId="end" placeholder="End Date" value={task?.endDate} onChange={(e) => setTask((prevState) => ({ ...prevState, endDate: e.value as string }))}></Calendar>
                        </div>
                        <div className="col-12 field">
